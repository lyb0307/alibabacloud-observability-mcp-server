...
class SLSToolkit:
    ...
    def _register_prompts(self):
        ...
        def query_sls_logs(question: str) -> str:
            ...
            return [
                ...
                base.UserMessage(
                    content="2.如果问题里面已经明确包含了查询语句，则直接使用"
                ),
                base.UserMessage(
    ...
    def _register_sls_tools(self):
        ...
        def sls_execute_sql_query(
            ...
            ## 错误处理
            - Column xxx can not be resolved: 可能存在查询列未开启统计，可以提示用户增加相对应的信息，或者调用 sls_describe_logstore 工具获取索引信息之后，要用户选择正确的字段或者提示用户对列开启统计。
